...
test('the agent is asked before any provider that needs a key', () => {
  const config: Config = { providers: ['agent', 'anthropic', 'openai'] };
...
test('a local endpoint needs no key at all', () => {
  const config: Config = { providers: ['openai'], baseUrl: 'http://localhost:11434/v1', model: 'llama3.2' };
  assert.equal(resolveSampler(config).name, 'openai');
});

test('no agent and no credentials gives an actionable error', () => {
  const config: Config = { providers: ['agent', 'openai'], apiKeyEnv: 'DEFINITELY_UNSET_KEY' };
...
test('a provider that cannot authenticate hands over to the next one', async () => {
  const config: Config = { providers: ['agent', 'openai'], baseUrl: 'http://localhost:1/v1', model: 'm' };
...
test('an agent that answers with nothing falls through instead of losing the session', async () => {
  ...
  await assert.rejects(
    extract('user: something', { project: 'p', config: { providers: ['agent'] }, agent: silent }),
    /No way to read the transcript[\s\S]*agent: the agent returned nothing/,
  );
});
...
test('a real provider error is surfaced, not swallowed by a fallback', async () => {
  ...
  await assert.rejects(
    extract('user: something', { project: 'p', config: { providers: ['agent'] }, agent: broken }),
    /client exploded/,
  );
});
