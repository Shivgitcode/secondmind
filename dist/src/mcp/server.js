...
export function buildServer(store = Store.open()) {
    const server = new McpServer({ name: 'secondmind', version: '0.1.0' }, { capabilities: { tools: {} }, enforceStrictCapabilities: true, instructions: INSTRUCTIONS });
    ...
    server.registerTool('search_context', {
        description: 'Search what was learned in earlier coding sessions — including sessions in other repositories. '
    ...
    server.registerTool('remember_context', {
        description: 'Save something worth knowing in a future session: a discovery, a decision, a dead end, or an open question. '
    ...
    server.registerTool('save_session', {
        description: 'Hand over the session transcript at the end of a piece of work. Pulls out the findings, '
    ...
    async ({ transcript, project, session_id: sessionId }, ctx) => {
        ...
        try {
            const { notes, via } = await extract(transcript, {
                project: name,
                config: loadConfig(),
                agent: clientCanSample() ? ctx : undefined,
            });
