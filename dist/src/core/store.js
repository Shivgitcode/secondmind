...
import { rank, toFtsQuery } from './ranking.js';
import { NOTE_TYPES, isNoteType } from './types.js';
...
const asText = (value) => (Array.isArray(value) ? value.join(' ') : (value ?? '')).trim();
/**
...
export class Store {
    ...
    search(query, options = {}) {
      ...
      ORDER BY bm25
      LIMIT ?
    `).all(match, CANDIDATE_POOL);
        const candidates = rows.map((row) => ({ ...toNote(row), bm25: row.bm25 }));
    ...
    stats() {
        const notes = this.db.prepare('SELECT count(*) AS n FROM notes').get();
        const projects = this.db.prepare('SELECT count(DISTINCT project) AS n FROM notes').get();
