...
import { isNoteType } from '../core/types.js';
import { readTranscript } from '../core/transcript.js';
import { extract } from '../extract/index.js';
import { browse } from './browse.js';
import { bold, dim, formatNotes } from './render.js';
const asList = (value) => (value ?? '').split(',').map((part) => part.trim()).filter(Boolean);
...
export function remember(store, words, options) {
    const content = words.join(' ');
    if (!content)
        throw new Error('Nothing to remember. Try: secondmind remember "what you learned"');
    const type = options.type && isNoteType(options.type) ? options.type : 'discovery';
    if (options.type && !isNoteType(options.type))
...
export function search(store, words, options) {
    const query = words.join(' ');
    if (!query)
        throw new Error('Nothing to search for. Try: secondmind search "payment timeout"');
...
export function list(store, options) {
    console.log(formatNotes(store.list({
        project: options.project ?? null,
        session: options.session ?? null,
        limit: options.limit ?? 20,
    }), { heading: 'Recent notes' }));
}
export async function compact(store, source, options) {
    ...
    for (const note of notes)
        console.log(`  ${note.type}: ${note.content}`);
}
export function forget(store, raw) {
    const id = Number(raw);
    if (!Number.isInteger(id))
        throw new Error('Which note? Try: secondmind forget 12');
...
export function browseCommand(store, options) {
    if (!process.stdin.isTTY)
        throw new Error('browse needs an interactive terminal. Use `secondmind list` instead.');
