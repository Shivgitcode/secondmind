...
async function main() {
    ...
    try {
        switch (command) {
            case 'remember':
                commands.remember(store, rest, options);
                return;
            case 'search':
                commands.search(store, rest, options);
                return;
            case 'list':
                commands.list(store, options);
                return;
            case 'browse':
                await commands.browseCommand(store, options);
                return;
            case 'compact':
                await commands.compact(store, rest[0] ?? '-', options);
                return;
            case 'forget':
                commands.forget(store, rest[0]);
                return;
            case 'stats':
