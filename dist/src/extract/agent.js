 ...
 */
export function agentSampler(ctx) {
    const requestSampling = ctx?.mcpReq?.requestSampling;
    if (typeof requestSampling !== 'function')
        return null;
    return {
        name: 'agent',
        ...
        async run(system, user) {
            ...
            if (!text.trim())
                throw new SamplerUnavailableError('agent', 'the agent returned nothing');
            return text;
        },
    };
}
