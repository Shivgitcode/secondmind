...
import { agentSampler } from './agent.js';
...
import { NoProviderError, SamplerUnavailableError } from './sampler.js';
...
export function buildSamplers(config, agent) {
    const builders = {
...
export function resolveSampler(config, agent) {
    const [first] = buildSamplers(config, agent);
    if (!first)
        throw new NoProviderError(config.providers);
    return first;
}
/**
 ...
 */
export async function extract(transcript, options) {
    if (!transcript.trim())
        return { notes: [], via: 'nothing to read' };
