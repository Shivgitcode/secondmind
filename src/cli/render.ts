...
export const bold = (s: string) => (useColor ? `\x1b[1m${s}\x1b[0m` : s);
export const invert = (s: string) => (useColor ? `\x1b[7m${s}\x1b[0m` : s);
 ...
 */
export function formatNotes(notes: Note[], options: { heading?: string } = {}): string {
  if (notes.length === 0) return 'Nothing relevant found in previous sessions.';
