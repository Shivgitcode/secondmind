 ...
 */
export function browse(store: Store, initialProject: string | null): Promise<void> {
  const out = process.stdout;
  const width = () => Math.max(40, out.columns || 80);
  const visibleNotes = () => Math.max(1, Math.floor(((out.rows || 24) - CHROME_ROWS) / ROWS_PER_NOTE));

  const state: State = { query: '', notes: [], selected: 0, expanded: false, pendingDelete: false, status: '' };
  ...
  const render = () => {
    ...
    if (state.notes.length === 0) {
      ...
      if (note) {
        ...
        lines.push('');
        for (const line of wrap(note.content, w - 3)) lines.push(`  ${line}`);
        if (note.files) lines.push('', dim(`  files: ${note.files}`));
        if (note.related) lines.push(dim(`  also relevant to: ${note.related}`));
        if (note.keywords) lines.push(dim(`  keywords: ${note.keywords}`));
      }
    } else {
    ...
    if (state.status) out.write(`   ${state.status}`);
  };
  ...
  return new Promise<void>((resolve) => {
    ...
    const onKey = (chunk: string | undefined, key: Key = {}) => {
      state.status = '';
      ...
      else if (key.name === 'down') state.selected = Math.min(state.notes.length - 1, state.selected + 1);
      else if (key.name === 'return' || key.name === 'enter') {
        state.expanded = !state.expanded && state.notes.length > 0;
        state.pendingDelete = false;
      }
      else if (key.name === 'backspace') { state.query = state.query.slice(0, -1); refresh(); }
