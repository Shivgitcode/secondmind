import type { Note, ScoredNote } from './types.js';
...
export interface RankInput extends Note {
  /** SQLite bm25 output: negative, and more negative means a better text match. */
  bm25: number;
}
 ...
 */
export function score(note: RankInput, project: string | null, now = Date.now()): number {
  let value = -note.bm25;

  if (project) {
    if (note.project === project) value *= SAME_PROJECT;
    else if (note.related.split(/\s+/).includes(project)) value *= RELATED_PROJECT;
  }

  value *= 1 + 0.25 * (note.importance - 2);
  if (note.source === 'saved') value *= HAND_WRITTEN;
...
export function rank(notes: RankInput[], project: string | null, limit: number): ScoredNote[] {
  const now = Date.now();
  return notes
 ...
 */
export function filterNotes<T extends Note>(notes: T[], query: string): T[] {
